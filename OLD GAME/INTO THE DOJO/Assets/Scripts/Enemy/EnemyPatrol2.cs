...
public class EnemyPatrol2 : MonoBehaviour {
	...
	private int nextDirection = 1; //Adjustor for reversing waypoint path
	private int current = 0; //Current node in path2
	...
	void Start () {
		wayPoints = GameObject.Find ("Waypoints 2").transform; //Finds and assigns the Waypoints' parent
															  //to the transform
	...
	void Waypointpath(){
		if ((seen1 == false) && (seen2 == false) && (seen3 == false) && (seen4 == false) && (seen5 == false)
			...
			if (distanceToWaypoint < 0.1) {
				...
				if (next >= wayPoints.childCount) {
					next = wayPoints.childCount - 1; //Decrement the next waypoint counter
					nextDirection = -1;
				} else if(next < 0){
	...
	void Move(){
		if (seenOnce == true) {
			//If no path2
			if (decompScript.path2 == null) {
				return;
			}
			
			//If reached the end of path2
			if (current >= decompScript.path2.Count) {
				return;
			}
			...
			Vector2 targetDirection = decompScript.path2[current].worldPos - new Vector2(transform.position.x, transform.position.y); //get direction of target
			toTarget = targetDirection.magnitude; //get horizontal distance	
			
			//If within next node's radius, move along path2
			if (toTarget > radius) {
				enemyAnim.SetTrigger("chase");
				
				if (decompScript.path2[current].worldPos.x >= transform.position.x) {
					//Move toward player
					transform.position = Vector3.MoveTowards (transform.position, decompScript.path2[current].worldPos,
					                                          moveSpeed * Time.deltaTime);
					//Face right
